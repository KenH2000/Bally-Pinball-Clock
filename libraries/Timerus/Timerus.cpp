...
Timerus::Timerus(void)
{
}

int8_t Timerus::every(unsigned long period, void (*callback)(), int repeatCount)
{
	...
	_events[i].lastEventTime = micros();
	_events[i].count = 0;
	return i;
}

int8_t Timerus::every(unsigned long period, void (*callback)())
{
	return every(period, callback, -1); // - means forever
}

int8_t Timerus::after(unsigned long period, void (*callback)())
{
	return every(period, callback, 1);
}

int8_t Timerus::oscillate(uint8_t pin, unsigned long period, uint8_t startingValue, int repeatCount)
{
	int8_t i = findFreeEventIndex();
...
int8_t Timerus::oscillate(uint8_t pin, unsigned long period, uint8_t startingValue)
{
 ...
 */
int8_t Timerus::pulse(uint8_t pin, unsigned long period, uint8_t startingValue)
{
 ...
 */
int8_t Timerus::pulseImmediate(uint8_t pin, unsigned long period, uint8_t pulseValue)
{
...
void Timerus::update(void)
{
	unsigned long now = micros();
	update(now);
}

void Timerus::update(unsigned long now)
{
...
int8_t Timerus::findFreeEventIndex(void)
{
	for (int8_t i = 0; i < MAX_NUMBER_OF_EVENTS; i++)
	{
		if (_events[i].eventType == EVENT_NONE)
